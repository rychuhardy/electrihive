...
class OptionsView(tkinter.PanedWindow):
    ...
    def __init__(self, root):
        ...
        self.files_labels = tkinter.Label(
            self, text='Description for Each vertex, edge or build cost should be in separate line.\nIntegers in each line should be separated with colon.')
        ...
        self.vertices_label = tkinter.Label(
            self, text='Select edges list first')
        ...
        self.neighbour_number_label = tkinter.Label(
            self, text='Number of neighbours in neighbourhood')
        self.neighbour_number_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validateNeighbourNumber)
        self.solution_liveness_label = tkinter.Label(
            self, text='Liveness of the solution')
        self.solution_liveness_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validateSolutionLiveness)
        self.bees_number_label = tkinter.Label(self, text='Number of bees')
        self.bees_number_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validateNumberOfBees)
        self.previous_solutions_label = tkinter.Label(
            self, text='Number of previous solutions in iteration')
        self.previous_solutions_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validatePreviousSolutionsNumber)
        self.max_iters_cond_label = tkinter.Label(
            self, text='Max number of iterations')
        self.max_iters_cond_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validateMaxIterationsNumber)
        self.min_cost_cond_label = tkinter.Label(
            self, text='Min cost of the solution')
        self.min_cost_cond_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validateMinSolutionCost)
        self.network_change_probability_label = tkinter.Label(
            self, text='Probability of network change/break')
        self.network_change_probability_entry = tkinter.Entry(
            self, validate="focusout", validatecommand=self.validateNetworkChangeProbability)
        ...
        self.network_change_probability_label.grid(
            row=10, column=0, **self.button_opts)
        self.network_change_probability_entry.grid(
            row=10, column=1, **self.button_opts)

        self.run_button.grid(row=11, column=0, columnspan=2,
                             padx=20, pady=40, sticky=tkinter.NSEW)
    ...
    def openGraphFile(self):
        ...
        if len(filename) == 0:
            return
        try:
            self.graph = read_edgelist(
                path=filename, delimiter=self.separator, data=(('cost', int),))
            ...
            tkinter.messagebox.showerror(
                title="File not found", message="The selected file was not found or could not be open")
    ...
    def openElectricityNeedsFile(self):
        try:
            ...
            for line in lines:
                ...
                try:
                    ...
                    tkinter.messagebox.showerror(
                        title="Expected numeric value", message="Expected integer values describing electricity needs for each vertex in line " + str(idx))
                    break
        except FileNotFoundError:
            tkinter.messagebox.showerror(
                title="File not found", message="The selected file was not found or could not be open")
        ...
        if set(vertices) <= set(self.graph.nodes()):
            ...
            tkinter.messagebox.showerror(
                title="Node values mismatch", message="The selected file specifies nodes that were not present in previous file\n")
        ...
        try:
            ...
            for line in lines:
                ...
                try:
                    ...
                    tkinter.messagebox.showerror(
                        title="Expected numeric value", message="Expected integer values describing build cost in line " + str(idx))
    ...
    def runAlgorithm(self):
        if self.files_chosen < 3:
            tkinter.messagebox.showerror(
                title="Select all files", message="Please select all three files.")
            return
        ...
        if self.validateNeighbourNumber():
            config['number_of_neighbours'] = int(
                self.neighbour_number_entry.get())
        else:
        ...
        if self.validatePreviousSolutionsNumber() and 'number_of_bees' in config and int(self.previous_solutions_entry.get()) < config['number_of_bees']:
            config['number_of_solutions'] = int(
                self.previous_solutions_entry.get())
        else:
        ...
        if self.validateNetworkChangeProbability():
            config['change_network_chance'] = int(
                self.network_change_probability_entry.get())
        else:
